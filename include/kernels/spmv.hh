...
class spmv {
public:
    spmv(const int iters) : iterations_(iters) {}
...
private:
    /** Perform any required steps before calling the spmv kernel that should
     * be timed. */
    virtual void preLoopRequirements() = 0;

    /** Perform the spmv kernel. */
    virtual void callSpmv() = 0;
    ...
    constexpr double calcChecksum() {
      // Checksum for spmv calculated by summing max and min element of output
      // vector
