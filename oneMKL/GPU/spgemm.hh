      ...
      if (offload_ == gpuOffloadType::always) {
        ...
        do {
          if (type_ == matrixType::rmat) {
            ...
            bandedDiagonalCSR<T, int64_t>(A_vals_store_, A_cols_store_, A_rows_store_, m_, k_, A_nnz_, SEED + seedOffset++);
            bandedDiagonalCSR<T, int64_t>(B_vals_store_, B_cols_store_, B_rows_store_, k_, n_, B_nnz_, SEED + seedOffset++);
          } else {
...
private:
    void preLoopRequirements() override {
      switch (offload_) {
        case gpuOffloadType::always: {
          // Nothing to do, does it all in the callSpgemm loop
          break;
        }
        case gpuOffloadType::once: {
