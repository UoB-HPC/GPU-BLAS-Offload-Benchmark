...
public:
  ...
  void initialise(gpuOffloadType offload, int m, int n, int k,
    ...
    index_ = rocsparse_indextype_i32;
  ...
  if (offload_ == gpuOffloadType::always) {
    ...
    do {
      if (type_ == matrixType::rmat) {
        ...
        bandedDiagonalCSR<T, int32_t>(A_vals_store_, A_cols_store_, A_rows_store_, m_, k_, A_nnz_, SEED + seedOffset++);
        bandedDiagonalCSR<T, int32_t>(B_vals_store_, B_cols_store_, B_rows_store_, k_, n_, B_nnz_, SEED + seedOffset++);
      } else {
  ...
  void callSpgemm() override {
    switch (offload_) {
      case gpuOffloadType::always: {
        ...
        rocCheckError(rocsparse_create_csr_descr(&A_descr_, m_, k_, A_nnz_, A_rows_dev_, A_cols_dev_, A_vals_dev_, index_, index_, base_, dataType_));

        rocCheckError(rocsparse_create_csr_descr(&B_descr_, k_, n_, B_nnz_, B_rows_dev_, B_cols_dev_, B_vals_dev_, index_, index_, base_, dataType_));

        rocCheckError(rocsparse_create_csr_descr(&C_descr_, m_, n_, 0, C_rows_dev_, nullptr, nullptr, index_, index_, base_, dataType_));
        size_t buffer_size = 0;
        void* temp_buffer;

        rocCheckError(rocsparse_spgemm(handle_, operation_, operation_, &alpha, A_descr_, B_descr_, &beta, C_descr_, C_descr_, dataType_, algorithm_, rocsparse_spgemm_stage_buffer_size, &buffer_size, nullptr));

        hipCheckError(hipMalloc(&temp_buffer, buffer_size));

        rocCheckError(rocsparse_spgemm(handle_, operation_, operation_, &alpha, A_descr_, B_descr_, &beta, C_descr_, C_descr_, dataType_, algorithm_, rocsparse_spgemm_stage_nnz, &buffer_size, temp_buffer));
        ...
        rocCheckError(rocsparse_spgemm(handle_, operation_, operation_, &alpha, A_descr_, B_descr_, &beta, C_descr_, C_descr_, dataType_, algorithm_, rocsparse_spgemm_stage_compute, &buffer_size, temp_buffer));
      ...
      case gpuOffloadType::unified: {
        ...
        rocCheckError(rocsparse_create_csr_descr(&A_descr_, m_, k_, A_nnz_, A_rows_, A_cols_, A_vals_, index_, index_, base_,dataType_));

        rocCheckError(rocsparse_create_csr_descr(&B_descr_, k_, n_, B_nnz_, B_rows_, B_cols_, B_vals_, index_, index_, base_, dataType_));

        rocCheckError(rocsparse_create_csr_descr(&C_descr_, m_, n_, 0, C_rows_32_, nullptr, nullptr, index_, index_, base_, dataType_));
        size_t buffer_size = 0;
        void* temp_buffer;
        ...
        hipCheckError(hipMallocManaged((void**)&C_cols_32_, sizeof(int32_t) * C_nnz_));
        hipCheckError(hipMallocManaged((void**)&C_vals_, sizeof(T) * C_nnz_));
        C_allocated_ = true;

        rocCheckError(rocsparse_csr_set_pointers(C_descr_, C_rows_32_, C_cols_32_, C_vals_));
  ...
  void postCallKernelCleanup() override {
    switch (offload_) {
      ...
      case gpuOffloadType::unified: {
        ...
        hipCheckError(hipFree(C_rows_32_));
