...
class spgemv_gpu : public spgemv<T> {
  ...
  void initialise(gpuOffloadType offload, int m, int n, 
    ...
    algorithm_ = rocsparse_spmv_alg_csr_adaptive;
  ...
  void callSpgemv() override {
    switch(offload_) {
      case gpuOffloadType::always: {
        ...
        size_t buffer_size;
        rocCheckError(rocsparse_spmv(handle_, operation_, &alpha, A_descr_, x_descr_, &beta, y_descr_, dataType_, algorithm_, rocsparse_spmv_stage_buffer_size, &buffer_size, nullptr));
        
        void* temp_buffer = nullptr;
        rocCheckError(rocsparse_spmv(handle_, operation_, &alpha, A_descr_, x_descr_, &beta, y_descr_, dataType_, algorithm_, rocsparse_spmv_stage_preprocess, &buffer_size, temp_buffer));

        rocCheckError(rocsparse_spmv(handle_, operation_, &alpha, A_descr_, x_descr_, &beta, y_descr_, dataType_, algorithm_, rocsparse_spmv_stage_compute, &buffer_size, temp_buffer));
        hipCheckError(hipDeviceSynchronize());
