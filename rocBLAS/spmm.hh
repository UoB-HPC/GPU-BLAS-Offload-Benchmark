...
public:
  ...
  void initialise(gpuOffloadType offload, int m, int n, int k,
    ...
    algorithm_ = rocsparse_spmm_alg_csr_merge_path;
    index_ = rocsparse_indextype_i64;
    base_ = rocsparse_index_base_zero;
    order_ = rocsparse_order_row;
  ...
  void toSparseFormat() override {
    if (offload_ == gpuOffloadType::always) {
      ...
      if (type_ == matrixType::rmat) {
        ...
        bandedDiagonalCSR<T, int64_t>(A_vals_store_, A_cols_store_, A_rows_store_, m_, k_, nnz_);
      } else {
        exit(1);
      }
    }
  ...
  void callSpmm() override {
    switch (offload_) {
      case gpuOffloadType::always: {
        ...
        rocCheckError(rocsparse_create_csr_descr(&A_descr_, m_, k_, nnz_, A_rows_dev_, A_cols_dev_, A_vals_dev_, index_, index_, base_, dataType_));
        
        rocCheckError(rocsparse_create_dnmat_descr(&B_descr_, k_, n_, n_, B_dev_, dataType_, order_));

        rocCheckError(rocsparse_create_dnmat_descr(&C_descr_, m_, n_, n_, C_dev_,
                                                   dataType_, order_));
        hipCheckError(hipDeviceSynchronize());

        size_t buffer_size = 0;
        rocCheckError(rocsparse_spmm(handle_, operation_, operation_, &alpha, A_descr_, B_descr_, &beta, C_descr_, dataType_, algorithm_, rocsparse_spmm_stage_buffer_size, &buffer_size, nullptr));
        
        void* buffer = nullptr;
        if (buffer_size > 0) hipCheckError(hipMalloc(&buffer, buffer_size));

        rocCheckError(rocsparse_spmm(handle_, operation_, operation_, &alpha, A_descr_, B_descr_, &beta, C_descr_, dataType_, algorithm_, rocsparse_spmm_stage_preprocess, &buffer_size, buffer));
        
        hipCheckError(hipDeviceSynchronize());
        rocCheckError(rocsparse_spmm(handle_, operation_, operation_, &alpha, A_descr_, B_descr_, &beta, C_descr_, dataType_, algorithm_, rocsparse_spmm_stage_compute, &buffer_size, buffer));
        ...
        hipCheckError(hipMemcpyAsync(C_, C_dev_, sizeof(T) * m_ * n_, hipMemcpyDeviceToHost, s3_));
        hipCheckError(hipDeviceSynchronize());
        break;
      }
      case gpuOffloadType::once: {
        ...
        rocCheckError(rocsparse_create_dnmat_descr(&C_descr_, m_, n_, n_, C_dev_, dataType_, order_));
        hipCheckError(hipDeviceSynchronize());
      ...
      case gpuOffloadType::unified: {
        // Set up the rocSPARSE structures for the GEMV
        rocCheckError(rocsparse_create_csr_descr(&A_descr_, m_, k_, nnz_, A_rows_, A_cols_, A_vals_, index_, index_, base_, dataType_));

        rocCheckError(rocsparse_create_dnmat_descr(&B_descr_, k_, n_, n_, B_, dataType_, order_));
        
        rocCheckError(rocsparse_create_dnmat_descr(&C_descr_, m_, n_, n_, C_, dataType_, order_));
        hipCheckError(hipDeviceSynchronize());
  ...
  void postLoopRequirements() override {
    switch (offload_) {
      ...
      case gpuOffloadType::unified: {
        ...
        hipCheckError(hipMemPrefetchAsync(C_, sizeof(T) * m_ * n_, hipCpuDeviceId, s3_));
